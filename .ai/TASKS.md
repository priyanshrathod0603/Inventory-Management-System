...
- [x] **PHASE 10** → Products + Inventory & Universal Business Onboarding (Universal Business Onboarding wizard with 10 industry categories, server-authoritative routing guards, resumable draft state, default warehouse provisioning, AppHeader & Settings personalization, Product master CRUD, SKUs, barcodes, categories hierarchy, brands, multi-warehouse stock, immutable StockMovement ledger, atomic StockAdjustment with negative protection, inter-warehouse StockTransfer state machine, batch & expiry tracking)